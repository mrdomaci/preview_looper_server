            ...
            <div class="col-md-6">
                ...
                <option
                    ...
                    @if ($infinite_repeat === 0)
                        selected 
                    @endif
                >{{ __('dynamic-preview-images.infinite_repeat_disabled') }}</option>
                <option
                    value="1"
                    @if ($infinite_repeat === 1)
                        selected 
                    @endif
                >{{ __('dynamic-preview-images.infinite_repeat_enabled') }}</option>
                </select>
            </div>
        </div>
        <div class="form-group row mt-4">
            <label for="settings_return_to_default" class="col-md-6 col-form-label">{{ __('dynamic-preview-images.return_to_default') }}:</label>
            <div class="col-md-6">
                <select class="form-control" name="settings_return_to_default" id="settings_return_to_default">
                    <option
                        ...
                        @if ($return_to_default === 0)
                            selected 
                        @endif
                    >{{ __('dynamic-preview-images.return_to_default_disabled') }}</option>
                    <option
                        value="1"
                        @if ($return_to_default === 1)
                            selected 
                        @endif
                    >{{ __('dynamic-preview-images.return_to_default_enabled') }}</option>
                </select>
            </div>
        </div>
        <div class="form-group row mt-4">
            <label for="settings_show_time" class="col-md-6 col-form-label">{{ __('dynamic-preview-images.show_time') }}:</label>
            <div class="col-md-6">
                <select class="form-control" name="settings_show_time" id="settings_show_time">
                    <option
                        ...
                        @if ($show_time == 1000)
                            selected 
                        @endif
                    >{{ __('dynamic-preview-images.show_time_short') }}</option>
                    <option
                        value="1500"
                        @if ($show_time == 1500)
                            selected 
                        @endif
                    >{{ __('dynamic-preview-images.show_time_medium') }}</option>
                    <option
                        value="2000"
                        @if ($show_time == 2000)
                            selected 
                        @endif
                    >{{ __('dynamic-preview-images.show_time_long') }}</option>
                </select>
            </div>
        </div>
        <div class="row mt-4">
